...
LANG_ALIASES = {
    "c++": "cpp", "cc": "cpp", "h": "cpp", "hpp": "cpp", "hxx": "cpp",
    "c": "c", "py": "python", "python3": "python",
    "js": "javascript", "jsx": "javascript", "ts": "javascript", "tsx": "javascript",
    "sh": "bash", "shell": "bash", "zsh": "bash", "console": "bash",
    "yml": "yaml", "text": "", "txt": "", "output": "", "plaintext": "",
}
...
class StreamPrinter:
    """Prose streams immediately. Fenced code becomes a dark panel live."""
    ...
    def finish(self) -> None:
        ...
        if self.inline != "n":
            marker = {"tick": "`", "star": "*", "italic": "*", "bold": "**", "bold2": "**"}[self.inline]
...
NON_CHAT_HINTS = (
    "embed", "rerank", "nvclip", "retrieve",
    "stable-diffusion", "sdxl", "flux", "imagen",
    "whisper", "parakeet", "tts", "asr",
    "cosmos", "edify",
)
...
def main():
    parser = argparse.ArgumentParser(description="NVIDIA AI API CLI")
    ...
    if not api_key:
        print(f"{C.RED}❌ NVIDIA_API_KEY not set{C.END}")
    ...
    help_text = (
        ...
        "  /quit              Exit (/exit, /bye, /q and Control+C also work)"
    )
    ...
    if RICH_AVAILABLE:
        console.print(Panel.fit("[bold cyan]NVIDIA AI API CLI[/bold cyan]", border_style="cyan"))
        console.print(
    ...
    while True:
        try:
            if RICH_AVAILABLE:
                ...
                user_input = input(f"{C.BLUE}{C.BOLD}You:{C.END} ").strip()
        ...
        if catalog.pick_pending and user_input.isdigit():
            handle_model_command(user_input, client, catalog)
            continue
